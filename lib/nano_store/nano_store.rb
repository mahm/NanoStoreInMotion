module NanoStore
  ...
  def self.store(type=:memory, path=nil)
    ...
    case type
    when :memory
      store = NSFNanoStore.createAndOpenStoreWithType(NSFNanoStore::NSFMemoryStoreType, path:nil, error: error_ptr)
    when :temporary, :temp
      store = NSFNanoStore.createAndOpenStoreWithType(NSFNanoStore::NSFTemporaryStoreType, path:nil, error: error_ptr)
    when :persistent, :file
      store = NSFNanoStore.createAndOpenStoreWithType(NSFNanoStore::NSFPersistentStoreType, path:path, error: error_ptr)
    else
